...
exports.injectTo = function (extensionId, isBackgroundPage, context) {
  ...
  chrome.runtime = {
    ...
    getManifest: function () {
      const manifest = ipcRendererUtils.invokeSync(ipcMessages.chrome.runtime.getManifest, extensionId)
      return manifest
    },
    ...
    connect (...args) {
      ...
      const { tabId, portId } = ipcRendererInternal.sendSync(chrome.runtime.connect, targetExtensionId, connectInfo)
    ...
    sendMessage (...args) {
      ...
      ipcRendererInternal.send(ipcMessages.chrome.runtime.sendMessage, targetExtensionId, message, originResultID)
      originResultID++
    },
