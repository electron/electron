...
class WebViewImpl {
  ...
  reset () {
    ...
    const newFrame = this.createBrowserPluginNode()
    const oldFrame = this.browserPluginNode
    this.browserPluginNode = newFrame
  ...
  onElementResize () {
    ...
    // is incorrect
    const newSize = {
      width: this.webviewNode.clientWidth,
      height: this.webviewNode.clientHeight
    }
  ...
  attachGuestInstance (guestInstanceId) {
    ...
    guestViewInternal.attachGuest(this.internalInstanceId, this.guestInstanceId, this.buildParams(), this.browserPluginNode.contentWindow)
    // ResizeObserver is a browser global not recognized by "standard".
    /* globals ResizeObserver */
    // TODO(zcbenz): Should we deprecate the "resize" event?
    this.resizeObserver = new ResizeObserver(this.onElementResize.bind(this)).observe(this.browserPluginNode)
  }
}
...
const registerWebViewElement = function () {
  ...
  proto.attachedCallback = function () {
    ...
    if (!internal.elementAttached) {
      ...
      if (instance) {
        if (internal.beforeFirstNavigation) {
