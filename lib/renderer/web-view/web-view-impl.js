...
const setupMethods = (WebViewElement) => {
  ...
  const createBlockHandler = function (method) {
    return function (...args) {
      const [error, result] = ipcRendererInternal.sendSync(ipcMessages.guestViewManager.methodSync, getGuestInstanceId(this), method, args)
      if (error) {
  ...
  const createNonBlockHandler = function (method) {
    return function (...args) {
      ...
      ipcRendererInternal.once(`${ipcMessages.guestViewManager.methodResult}_${requestId}`, function (event, error, result) {
        if (error == null) {
      ...
      ipcRendererInternal.send(ipcMessages.guestViewManager.method, requestId, getGuestInstanceId(this), method, args, callback != null)
    }
  }
