...
for (let arg of process.argv) {
  if (arg.indexOf('--guest-instance-id=') === 0) {
    ...
    preloadScripts = arg.substr(arg.indexOf('=') + 1).split(',')
  }
}
...
// Load the scripts specfied by the "preload" attribute.
if (preloadScripts) {
  for (var script of preloadScripts) {
    try {
      require(script)
    } catch (error) {
