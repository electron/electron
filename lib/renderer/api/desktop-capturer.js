...
const getSources = (options) => {
  return new Promise((resolve, reject) => {
    ...
    ipcRendererInternal.send(ipcMessages.desktopCapturer.getSources, captureWindow, captureScreen, options.thumbnailSize, options.fetchWindowIcons, id)
    return ipcRendererInternal.once(`${ipcMessages.desktopCapturer.getSourcesResult}_${id}`, (event, sources) => {
      try {
