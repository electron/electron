...
function handleMessage (channel, handler) {
  ipcRendererInternal.on(channel, (event, passedContextId, id, ...args) => {
    if (passedContextId === contextId) {
      ...
      ipcRendererInternal.send(ipcMessages.remote.browser.wrongContextError, contextId, passedContextId, id)
    }
  })
}

// Browser calls a callback in renderer.
handleMessage(ipcMessages.remote.renderer.callbackInvoke, (id, args) => {
  callbacksRegistry.apply(id, metaToValue(args))
})

// A callback in browser is released.
handleMessage(ipcMessages.remote.renderer.callbackRelease, (id) => {
  callbacksRegistry.remove(id)
})

exports.require = (module) => {
  const command = ipcMessages.remote.browser.require
...
exports.getBuiltin = (module) => {
  const command = ipcMessages.remote.browser.getBuiltin
  const meta = ipcRendererInternal.sendSync(command, contextId, module)
  return metaToValue(meta)
}

exports.getCurrentWindow = () => {
  const command = ipcMessages.remote.browser.getCurrentWindow
...
exports.getCurrentWebContents = () => {
  const command = ipcMessages.remote.browser.getCurrentWebContents
  const meta = ipcRendererInternal.sendSync(command, contextId)
  return metaToValue(meta)
}
...
exports.getGlobal = (name) => {
  const command = ipcMessages.remote.browser.getGlobal
...
exports.getGuestWebContents = (guestInstanceId) => {
  const command = ipcMessages.remote.browser.getGuestWebContents
