...
handleMessage(ipcMessages.guestViewManager.createGuest, function (event, params, requestId) {
  event._replyInternal(`ELECTRON_RESPONSE_${requestId}`, createGuest(event.sender, params))
})

handleMessage(ipcMessages.guestViewManager.createGuestSync, function (event, params) {
  event.returnValue = createGuest(event.sender, params)
})

handleMessage(ipcMessages.guestViewManager.destroyGuest, function (event, guestInstanceId) {
  try {
...
handleMessage(ipcMessages.guestViewManager.attachGuest, function (event, embedderFrameId, elementInstanceId, guestInstanceId, params) {
  try {
...
ipcMainInternal.on(ipcMessages.guestViewManager.focusChange, function (event, focus, guestInstanceId) {
...
handleMessage(ipcMessages.guestViewManager.method, function (event, requestId, guestInstanceId, method, args, hasCallback) {
  new Promise(resolve => {
    const guest = getGuestForWebContents(guestInstanceId, event.sender)
    ...
    event._replyInternal(`${ipcMessages.guestViewManager.methodResult}_${requestId}`, ...responseArgs)
  })
})

handleMessage(ipcMessages.guestViewManager.methodSync, function (event, guestInstanceId, method, args) {
  try {
    const guest = getGuestForWebContents(guestInstanceId, event.sender)
