...
app.makeSingleInstance = function (callback) {
  return makeSingleInstance.call(app, function (argv, ...args) {
    if (process.platform === 'win32') {
      ...
      setImmediate(() => {
        argv.forEach((arg) => {
          const protocolMatch = arg.match(/^([\s\S]+):\/\/[\s\S]+/)
          if (protocolMatch && app.getDefaultProtocolClient(protocolMatch[1]).split(/ /g)[0].replace(/^"(.+)"$/g, '$1') === process.execPath) {
            app.emit('open-url', { preventDefault: () => {} }, arg)
          }
        })
      })
    }
