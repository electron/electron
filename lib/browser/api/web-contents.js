...
class BrowserSideMessagePort extends EventEmitter {
  constructor (internalPort) {
    super()
    this._internalPort = internalPort
    this._internalPort.emit = (channel, event) => {
      if (channel === 'message') { event = { ...event, ports: event.ports.map(p => new BrowserSideMessagePort(p)) } }
  ...
  postMessage (message, options) {
    if (Array.isArray(options)) {
      options = options.map(o => o instanceof BrowserSideMessagePort ? o._internalPort : o)
    }
    return this._internalPort.postMessage(message, options)
  }
}
...
WebContents.prototype.postMessage = function (channel, message, options) {
  if (Array.isArray(options)) {
    options = options.map(o => o instanceof BrowserSideMessagePort ? o._internalPort : o)
  }
  this._postMessage(channel, message, options)
}
...
WebContents.prototype._init = function () {
  ...
  this.on('-ipc-ports', function (event, internal, channel, message, ports) {
    event.ports = ports.map(p => new BrowserSideMessagePort(p))
