...
class ClientRequest extends Writable {
  ...
  setHeader (name, value) {
    ...
    this._urlLoaderOptions.extraHeaders[key] = value.toString()
  }
  ...
  _startRequest () {
    ...
    this._urlLoader.on('redirect', (event, redirectInfo, responseHead, followRedirect) => {
      const cb = () => { followRedirect([]) }
      const handled = this.emit('redirect', redirectInfo.statusCode, redirectInfo.newMethod, redirectInfo.newUrl, cb)
      if (!handled) {
