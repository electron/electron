...
class ClientRequest extends Writable {
  ...
  _startRequest () {
    ...
    this._urlLoader.on('redirect', (event, redirectInfo, headers, followRedirect) => {
      this._followRedirectCb = () => { followRedirect([]); this._followRedirectCb = null }
      const { statusCode, newMethod, newUrl } = redirectInfo
      const handled = this.emit('redirect', statusCode, newMethod, newUrl, headers)
      if (!handled) {
