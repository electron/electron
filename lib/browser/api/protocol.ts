import { getFetchResponseInfo } from '@electron/internal/browser/api/net-fetch';
  ...
  const success = register.call(this, scheme, async (preq: ProtocolRequest, cb: any) => {
    try {
      ...
      if (!validateResponse(res)) {
        ...
        const fetchResponse = getFetchResponseInfo(res);
        const head = {
        ...
        if (fetchResponse?.canTransfer) {
          cb({ loader: fetchResponse.loader, ...head });
        } else {
