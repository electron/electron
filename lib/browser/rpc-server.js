      ...
      case 'function': {
        ...
        const callIntoRenderer = function (...args) {
          ...
          if (!sender.isDestroyed()) {
            succeed = sender._sendToFrameInternal(frameId, ipcMessages.remote.renderer.callbackInvoke, contextId, meta.id, valueToMeta(sender, contextId, args))
          }
          if (!succeed) {
...
ipcMainUtils.handleSync(ipcMessages.clipboard.readFindText, function (event) {
  return electron.clipboard.readFindText()
})

ipcMainUtils.handleSync(ipcMessages.clipboard.writeFindText, function (event, text) {
...
ipcMainInternal.on(ipcMessages.sandboxLoad, function (event) {
  const preloadPaths = [
...
ipcMainInternal.on(ipcMessages.preloadError, function (event, preloadPath, error) {
