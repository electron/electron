...
// Detect AppIndicator (electron/electron#10427)
function * hasAppIndicator (sessionBus) {
  yield new Promise(resolve => sessionBus.nameHasOwner('org.kde.StatusNotifierWatcher', (err, has) => {
    if (err) {
      resolve(false)
    } else if (has) {
      resolve(true)
...
try {
  ...
  if (hasAppIndicator(sessionBus).next()) {
    process.env.XDG_CURRENT_DESKTOP = 'Unity'
  } else {
  }
} catch (e) {
  console.log('error while detecting AppIndicator support: ' + e)
}
