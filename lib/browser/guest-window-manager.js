...
ipcMainInternal.on(ipcMessages.guestWindowManager.windowOpen, (event, url, frameName, features) => {
...
ipcMainInternal.on(ipcMessages.guestWindowManager.windowClose, function (event, guestId) {
...
ipcMainInternal.on(ipcMessages.guestWindowManager.windowMethod, function (event, guestId, method, ...args) {
  const guestContents = webContents.fromId(guestId)
...
ipcMainInternal.on(ipcMessages.guestWindowManager.webContentsPostMessage, function (event, guestId, message, targetOrigin, sourceOrigin) {
...
ipcMainInternal.on(ipcMessages.guestWindowManager.webContentsMethod, function (event, guestId, method, ...args) {
  const guestContents = webContents.fromId(guestId)
  if (guestContents == null) return
...
ipcMainInternal.on(ipcMessages.guestWindowManager.webContentsMethodSync, function (event, guestId, method, ...args) {
  const guestContents = webContents.fromId(guestId)
  if (guestContents == null) {
    event.returnValue = null
    return
