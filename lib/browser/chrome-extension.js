...
ipcMainUtils.handleSync('CHROME_GET_MESSAGES', function (event, extensionId, language) {
  assertChromeDevTools(event.sender, 'chrome.i18n.getMessage()')
...
ipcMainUtils.handle('CHROME_STORAGE_READ', async function (event, storageType, extensionId) {
  assertChromeDevTools(event.sender, 'chrome.storage.*.get()')
...
ipcMainUtils.handle('CHROME_STORAGE_WRITE', async function (event, storageType, extensionId, data) {
  assertChromeDevTools(event.sender, 'chrome.storage.*.set()')
...
ipcMain.on('CHROME_TABS_EXECUTESCRIPT', function (event, requestId, tabId, extensionId, details) {
  try {
    assertChromeDevTools(event.sender, 'chrome.tabs.executeScript()')
