...
ipcMainInternal.on(ipcMessages.chrome.runtime.connect, function (event, extensionId, connectInfo) {
...
ipcMainUtils.handleSync(ipcMessages.chrome.runtime.getManifest, function (event, extensionId) {
...
let resultID = 1
ipcMainInternal.on(ipcMessages.chrome.runtime.sendMessage, function (event, extensionId, message, originResultID) {
  const page = backgroundPages[extensionId]
  if (!page) {
    console.error(`Connect to unknown extension ${extensionId}`)
    return
