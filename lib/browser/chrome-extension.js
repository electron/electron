...
const allowedTabPropertyMethods = new Set([
  'isFocused',
  'isCurrentlyAudible',
  'getURL',
  'getTitle',
...
ipcMainUtils.handle('CHROME_TABS_PROPERTY_GET', (event, tabId, getMethod) => {
  if (!allowedTabPropertyMethods.has(getMethod)) {
    console.error(`Blocked ${event.sender.id} from calling webContents method ${getMethod} on webContents ${tabId}`)
    return
  }
  ...
  return contents[getMethod]()
})

ipcMainUtils.handle('CHROME_TABS_QUERY', async (event, info) => {
  const allContents = webContents.getAllWebContents()
  const isSet = (key) => typeof info[key] !== 'undefined'

  return allContents
    .filter((contents) => {
      if (contents.isDestroyed()) return false

      if (isSet('active') && contents.isFocused() !== info.active) return false
      // Electron does not support pinned tabs
      if (isSet('pinned') && info.pinned) return false
      if (isSet('audible') && contents.isCurrentlyAudible() !== info.audible) return false
      if (isSet('muted') && contents.isAudioMuted() !== info.muted) return false
      // Electron does not support highlighted tabs
      if (isSet('highlighted') && info.highlighted) return false
      if (isSet('autoDiscardable') && info.autoDiscardable) return false
      // Electron does not support currentWindow or lastFocusedWindow
      if (isSet('currentWindow')) return false
      if (isSet('lastFocusedWindow')) return false
      if (isSet('status')) {
        if (info.status === 'complete' && contents.isLoadingMainFrame()) return false
        if (info.status === 'loading' && !contents.isLoadingMainFrame()) return false
        if (info.status !== 'complete' && info.status !== 'loading') return false
      }
      if (isSet('title') && contents.getTitle() !== info.title) return false
      if (isSet('index') && info.index !== 0) return false
      if (isSet('windowType')) {
        switch (info.windowType) {
          case 'normal':
          case 'app':
            if (contents.getType() === 'remote') return false
            break
          case 'popup':
          case 'panel':
            return false
