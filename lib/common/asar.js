...
(function() {
  ...
  const overrideAPI = function(module, name, pathArgumentIndex) {
    ...
    if (old[util.promisify.custom]) {
      module[name][util.promisify.custom] = function(...args) {
        const pathArgument = args[pathArgumentIndex]
        const {isAsar, asarPath, filePath} = splitPath(pathArgument)
        if (!isAsar) return old[util.promisify.custom].apply(this, args)
        ...
        args[pathArgumentIndex] = newPath
        return old[util.promisify.custom].apply(this, args)
      }
    }
  }
  ...
  exports.wrapFsWithAsar = (fs) => {
    ...
    fs.realpathSync = function(pathArgument, options, ...args) {
      const {isAsar, asarPath, filePath} = splitPath(pathArgument)
      if (!isAsar) return realpathSync.apply(this, [pathArgument, options, ...args])
    ...
    fs.realpathSync.native = function(pathArgument, options, ...args) {
      const {isAsar, asarPath, filePath} = splitPath(pathArgument)
      if (!isAsar) return realpathSync.native.apply(this, [pathArgument, options, ...args])
    ...
    const {realpath} = fs
    fs.realpath = function(pathArgument, options, callback, ...args) {
      const {isAsar, asarPath, filePath} = splitPath(pathArgument)
      if (!isAsar) return realpath.apply(this, [pathArgument, options, callback, ...args])
    ...
    fs.realpath.native = function(pathArgument, options, callback, ...args) {
      const {isAsar, asarPath, filePath} = splitPath(pathArgument)
      if (!isAsar) return realpath.native.apply(this, [pathArgument, options, callback, ...args])
    ...
    const {access} = fs
    fs.access = function(pathArgument, mode, callback, ...args) {
      const {isAsar, asarPath, filePath} = splitPath(pathArgument)
      if (!isAsar) return access.apply(this, [pathArgument, mode, callback, ...args])
    ...
    const {accessSync} = fs
    fs.accessSync = function(pathArgument, mode, ...args) {
      const {isAsar, asarPath, filePath} = splitPath(pathArgument)
      if (!isAsar) return accessSync.apply(this, [pathArgument, mode, ...args])
    ...
    const {readFile} = fs
    fs.readFile = function(pathArgument, options, callback, ...args) {
      const {isAsar, asarPath, filePath} = splitPath(pathArgument)
      if (!isAsar) return readFile.apply(this, [pathArgument, options, callback, ...args])
    ...
    fs.readFileSync = function(pathArgument, options, ...args) {
      const {isAsar, asarPath, filePath} = splitPath(pathArgument)
      if (!isAsar) return readFileSync.apply(this, [pathArgument, options, ...args])
    ...
    const {readdir} = fs
    fs.readdir = function(pathArgument, options, callback, ...args) {
      const {isAsar, asarPath, filePath} = splitPath(pathArgument)
      if (typeof options === 'function') {
        callback = options
        options = {}
      }
      if (!isAsar) return readdir.apply(this, [pathArgument, options, callback, ...args])
    ...
    fs.readdirSync = function(pathArgument, options, ...args) {
      const {isAsar, asarPath, filePath} = splitPath(pathArgument)
      if (!isAsar) return readdirSync.apply(this, [pathArgument, options, ...args])
