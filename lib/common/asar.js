...
(function () {
  ...
  exports.wrapFsWithAsar = fs => {
    ...
    const { statSync } = fs
    fs.statSync = (pathArgument, ...args) => {
      const { isAsar } = splitPath(pathArgument)
      if (!isAsar) return statSync(pathArgument, ...args)

      // Do not distinguish links for now.
      return fs.lstatSync(pathArgument, ...args)
    }

    const { stat } = fs
    fs.stat = (pathArgument, ...args) => {
      const { isAsar } = splitPath(pathArgument)
      if (!isAsar) return stat(pathArgument, ...args)

      // Do not distinguish links for now.
      process.nextTick(() => fs.lstat(pathArgument, ...args))
    }
