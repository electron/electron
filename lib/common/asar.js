...
(function () {
  ...
  exports.wrapFsWithAsar = fs => {
    ...
    // We will eagily override the child_process APIs when this env var is set so that stack traces
    // generated inside node unit tests will match.  This env var will only slow things down in users
    // apps and should not be used.
    if (process.env.ELECTRON_EAGER_ASAR_HOOK) {
