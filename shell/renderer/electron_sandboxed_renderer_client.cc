...
void ElectronSandboxedRendererClient::InitializeBindings(
  ...
  // ElectronFrameStartup at frame creation and ahead of each
  // CommitNavigation, so even a script context forced onto the initial empty
  // document observes data. If an unexpected edge case still reaches the
  // bundle without it, the bundle warns and skips preloads instead of
  // throwing.
