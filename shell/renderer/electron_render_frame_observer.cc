...
void ElectronRenderFrameObserver::DidInstallConditionalFeatures(
  ...
  DeferMicrotasksScope defer_microtasks(isolate, context->GetMicrotaskQueue());
  ...
  // Like the rest of the set-up, the window tweaks go in once per context.
  const bool window_needs_setup =
      is_main_world(world_id)
          ? needs_main_world_setup
          : is_isolated_world(world_id) && client_notified_context_ != context;
  if (window_needs_setup &&
