...
bool ElectronRuntimeAPIDelegate::GetPlatformInfo(PlatformInfo* info) {
  std::string_view os = update_client::UpdateQueryParams::GetOS();
  if (os == "mac") {
  ...
  std::string_view arch = update_client::UpdateQueryParams::GetArch();
  if (arch == "arm") {
  ...
  std::string_view nacl_arch = update_client::UpdateQueryParams::GetNaclArch();
  if (nacl_arch == "arm") {
