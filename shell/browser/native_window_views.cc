...
NativeWindowViews::NativeWindowViews(const gin_helper::Dictionary& options,
  ...
  // When resizable_ is false, WS_THICKFRAME is not passed into either
  // call, so the Windows 7 frame still shows. One workaround would be
  // to call set_can_resize(true) here, and then move the
  // SetCanResize(resizable_) call after the SetWindowLong call
  // around line 365, but that's a much larger change.
  set_can_resize(true);
