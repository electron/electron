...
void WindowStateWatcher::OnEvent(const x11::Event& x11_event) {
  if (IsWindowStateEvent(x11_event)) {
    ...
    if (ui::GetAtomArrayProperty(static_cast<x11::Window>(window_->GetAcceleratedWidget()),
                             "_NET_WM_STATE", &wm_states)) {
      auto props = base::flat_set<x11::Atom>(std::begin(wm_states), std::end(wm_states));
      bool is_minimized = props.find(gfx::GetAtom("_NET_WM_STATE_HIDDEN")) != props.end();
      bool is_maximized = props.find(gfx::GetAtom("_NET_WM_STATE_MAXIMIZED_VERT")) != props.end() && props.find(gfx::GetAtom("_NET_WM_STATE_MAXIMIZED_HORZ")) != props.end();
      bool is_fullscreen = props.find(gfx::GetAtom("_NET_WM_STATE_FULLSCREEN")) != props.end();
