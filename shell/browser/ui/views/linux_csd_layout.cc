...
LinuxCSDLayout::LinuxCSDLayout(NativeWindowViews* window) : window_(window) {
  host_supports_client_frame_shadow_ = SupportsClientFrameShadow();
}

gfx::Insets LinuxCSDLayout::RestoredFrameBorderInsets() const {
...
gfx::Insets LinuxCSDLayout::GetInputInsets() const {
...
bool LinuxCSDLayout::SupportsClientFrameShadow() const {
...
void LinuxCSDLayout::PaintWindowFrame(gfx::Canvas* canvas,
...
int LinuxCSDLayout::GetTranslucentTopAreaHeight() const {
  return 0;
}

ui::WindowFrameProvider* LinuxCSDLayout::GetFrameProvider() const {
