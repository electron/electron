...
void OpaqueFrameView::Layout(PassKey) {
  ...
  leading_button_placeholder_container_->SetBounds(
      client_bounds.x(), client_bounds.y(), minimum_size_for_leading_buttons_,
      height);
  trailing_button_placeholder_container_->SetBounds(
      trailing_x, client_bounds.y(), minimum_size_for_trailing_buttons_,
      height);
  LayoutWindowControlsOverlay();
}
...
void OpaqueFrameView::LayoutWindowControlsOverlay() {
  ...
  auto bounding_rect =
      gfx::Rect(leading_button_placeholder_container_->bounds().width(), 0,
                titlebar_width, overlay_height);
...
void OpaqueFrameView::SetBoundsForButton(views::FrameButton button_id,
  ...
  switch (alignment) {
    case ALIGN_LEADING: {
      ...
      minimum_size_for_leading_buttons_ +=
          button_size.width() + button_end_spacing;
    ...
    case ALIGN_TRAILING: {
      ...
      minimum_size_for_trailing_buttons_ +=
          button_size.width() + button_end_spacing;
