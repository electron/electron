...
void NativeWindowViews::SetForwardMouseMessages(bool forward) {
  if (forward && !forwarding_mouse_messages_) {
    ...
    // Register the message used to signal a genuine mouse leave before
    // installing the subclass and hook that recognize it. RegisterWindowMessage
    // is idempotent for the same string, so re-enabling forwarding is safe.
    forwarded_mouse_leave_message_ =
        RegisterWindowMessage(TEXT("ForwardedMouseLeave"));

    POINT p;
    GetCursorPos(&p);
    was_forwarded_mouse_in_window_ =
        PointToClientAndInRect(GetNativeWindowHandle(), &p);
...
// the point is inside the window rect.
BOOL NativeWindowViews::PointToClientAndInRect(HWND hwnd, POINT* pt) {
  RECT client_rect;
...
LRESULT CALLBACK NativeWindowViews::SubclassProc(HWND hwnd,
  ...
  if (msg == window->forwarded_mouse_leave_message_) {
    // Our hook posted this to signal a genuine leave while forwarding.
    // Translate it into a normal WM_MOUSELEAVE so Chromium processes it just
    // like an OS-generated leave.
    return DefSubclassProc(hwnd, WM_MOUSELEAVE, 0, 0);
  }

  switch (msg) {
    case WM_MOUSELEAVE: {
      // When input is forwarded to underlying windows, the OS sends
      // WM_MOUSELEAVE messages (generated by Chromium's own TrackMouseEvent).
      // If not handled, they interfere with Chromium logic, causing for
      // example mouseleave events to fire. If those events are used to exit
      // forward mode, excessive flickering on for example hover items in
      // underlying windows can occur due to rapidly entering and leaving
      // forwarding mode. By consuming and ignoring the message, we're
      // essentially telling Chromium that we have not left the window despite
      // somebody else getting the messages. As to why this is caught for the
      // legacy window and not the actual browser window is simply that the
      // legacy window somehow makes use of these events; posting to the main
      // window didn't work.
...
LRESULT CALLBACK NativeWindowViews::MouseHookProc(int n_code,
  ...
  if (w_param == WM_MOUSEMOVE) {
    for (auto* window : *forwarding_windows_) {
      ...
      // Decide how to route the move and whether to dispatch a leave.
      //
      // Every cursor position inside the parent window is routed to the legacy
      // window, including positions over parent-only regions (menu bar, WCO
      // buttons, docked DevTools). The legacy window maps the coordinates into
      // the parent's space and forwards them via
      // HWNDMessageHandler::HandleMouseMessage, which does not re-arm
      // TrackMouseEvent on the parent. This drives hover state for both the
      // web contents and the parent's own UI without ever posting directly to
      // the parent (which would arm its tracking and produce spurious or stuck
      // WM_MOUSELEAVE on a window we don't subclass).
      //
      // `was_forwarded_mouse_in_window_` records whether the cursor was last
      // seen inside the window. When the cursor leaves the window entirely,
      // post the forwarded-mouse-leave custom message so the leave reaches
      // Chromium exactly once (see SubclassProc).
      if (PointToClientAndInRect(window_hwnd, &p)) {
        window->was_forwarded_mouse_in_window_ = true;
        POINT child_p = reinterpret_cast<MSLLHOOKSTRUCT*>(l_param)->pt;
        RECT child_rect;
        GetClientRect(window->legacy_window_, &child_rect);
        ScreenToClient(window->legacy_window_, &child_p);
        ...
        // The cursor left the parent window entirely (it may have left both
        // the parent and the legacy window at once). Make sure the legacy
        // window receives the leave so hover state is cleared.
        window->was_forwarded_mouse_in_window_ = false;
        // SubclassProc translates this custom message into a WM_MOUSELEAVE.
        PostMessage(window->legacy_window_,
                    window->forwarded_mouse_leave_message_, 0, 0);
      }
    }
  }
