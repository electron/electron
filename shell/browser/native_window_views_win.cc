...
void NativeWindowViews::EnsureLegacyWindowSubclass(HWND legacy_window) {
  ...
  if (legacy_window != active_legacy_window_) {
    if (active_legacy_window_) {
      ...
      RemoveWindowSubclass(active_legacy_window_, SubclassProc, 1);
    }
    if (legacy_window) {
...
HWND NativeWindowViews::GetPrimaryWebContentsViewLegacyWindow() {
  // The primary web contents view is only registered by BrowserWindow, and the
  // tracked view is cleared as soon as it is destroyed, so every hop below can
  // legitimately be missing (e.g. for BaseWindow, or while the view is being
  // torn down). Returning nullptr is a supported outcome: the callers treat it
  // as "no legacy window available".
  auto* primary_view = primary_web_contents_view();
  if (!primary_view)
    return nullptr;

  auto* inspectable_web_contents = primary_view->inspectable_web_contents();
  // Non-null by construction: the view is owned by its InspectableWebContents
  // and holds a back pointer to it.
