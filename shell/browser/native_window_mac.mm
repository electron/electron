...
// at a lower Z-order (behind it) so that user-added child views paint above the
// web content. However, views::ViewTargeterDelegate::TargetForRect iterates
// children front-to-back and returns the first child whose HitTestRect() is
// true. content_view_ covers the full window area, so the WebContentsView (and
// its NativeViewHost) is never reached.
//
