...
class WebContents final : public ExclusiveAccessContext,
 ...
 private:
  ...
  // factor from physical primary screen's info. In later version after the
  // breaking change notice, we need to set this to 1.0f so that the offscreen
  // rendering use 1.0 as default when `deviceScaleFactor` is not specified in
  // webPreferences.
