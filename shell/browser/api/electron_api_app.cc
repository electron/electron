...
void App::EnableAccessibilitySupportFeatures(
    gin_helper::ErrorThrower thrower,
    const std::vector<std::string>& features) {
  if (!Browser::Get()->is_ready()) {
    thrower.ThrowError(
        "app.enableAccessibilitySupportFeatures() can only be called after app "
        "is ready");
    return;
  }
...
gin::ObjectTemplateBuilder App::GetObjectTemplateBuilder(v8::Isolate* isolate) {
  ...
  return gin_helper::EventEmitterMixin<App>::GetObjectTemplateBuilder(isolate)
      ...
      .SetMethod("enableAccessibilitySupportFeatures",
                 &App::EnableAccessibilitySupportFeatures)
