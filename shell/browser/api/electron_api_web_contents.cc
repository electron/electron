...
void OnPrinterResolved(base::WeakPtr<content::WebContents> web_contents,
  ...
  print_view_manager->Print(rfh, std::move(print_settings), silent,
...
void WebContents::Print(std::optional<base::DictValue> options,
  ...
  for (const base::Value& range : ranges ? *ranges : no_ranges) {
    // Chromium's page ranges are 1-based.
    page_ranges.Append(base::DictValue()
                           .Set(printing::kSettingPageRangeFrom,
                                integer(range.GetDict(), "from") + 1)
                           .Set(printing::kSettingPageRangeTo,
                                integer(range.GetDict(), "to") + 1));
  }
  if (!page_ranges.empty())
    settings.Set(printing::kSettingPageRange, std::move(page_ranges));
  ...
  ResolvePrinter(
      ...
      base::BindOnce(&OnPrinterResolved, web_contents()->GetWeakPtr(),
                     std::move(settings), boolean("silent", false),
                     use_printer_default_page_size, std::move(callback)));
}
