...
#endif
{
  ...
  if (is_guest()) {
    ...
    params.enable_wake_locks = !disable_wake_locks;
    base::AutoReset<bool> spare_compatible(
        ElectronBrowserClient::Get()->creating_spare_compatible_web_contents(),
        WebContentsPreferences::CanUseSpareRenderer(options));
    web_contents = content::WebContents::Create(params);
  }
