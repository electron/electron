...
void Message(v8::Isolate* isolate,
  ...
  v8::HandleScope handle_scope(isolate);
  v8::Local<v8::Object> event;
  if (!ipc_event->GetWrapper(isolate).ToLocal(&event))
    return;
...
void Invoke(v8::Isolate* isolate,
  ...
  if (internal) {
    ...
    CollectFrameTargets(context, sender_wrapper,
                        ipc_event->frame_tree_node_id(), &targets);
  }
...
void ServiceWorkerMessage(v8::Isolate* isolate,
                          api::IpcMainServiceWorkerEvent* ipc_event,
                          bool internal,
                          const std::string& channel,
                          v8::Local<v8::Value> args,
                          bool sync) {
  ...
  if (!ipc_event->GetWrapper(isolate).ToLocal(&event))
    return;
  api::Session* session = ipc_event->session();
  if (!session)
    return;
