...
void ProxyingWebSocket::OnAuthRequiredComplete(WebRequestAPI::AuthRequiredResponse rv) {
  CHECK(auth_required_callback_);
  ResumeIncomingMethodCallProcessing();
  switch (rv) {
    case WebRequestAPI::AuthRequiredResponse::AUTH_REQUIRED_RESPONSE_NO_ACTION:
    case WebRequestAPI::AuthRequiredResponse::AUTH_REQUIRED_RESPONSE_CANCEL_AUTH:
...
void ProxyingWebSocket::OnHeadersReceivedCompleteForAuth(
  ...
  auto auth_rv = web_request_api_->OnAuthRequired(&info_, auth_info, std::move(continuation),
                                                  &auth_credentials_);
  PauseIncomingMethodCallProcessing();
  if (auth_rv == WebRequestAPI::AuthRequiredResponse::AUTH_REQUIRED_RESPONSE_IO_PENDING) {
    return;
  }
