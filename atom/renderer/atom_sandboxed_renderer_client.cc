...
void AtomSandboxedRendererClient::DidCreateScriptContext(
  ...
  std::string left = "(function(binding, preloadPath, preloadSrc, require) {\n";
  std::string right = "\n})";
  ...
  if (!preload_script_path.empty()) {
    IPC::SyncMessage* message = new AtomViewHostMsg_GetPreloadScript(
        render_frame->GetRoutingID(), &preload_script);
    if (!render_frame->Send(message)) {
      PLOG(WARNING) << "Unable to send AtomViewHostMsg_GetPreloadScript";
    }
  }
