...
* `appCache` The directory for storing your app's Electron cache, which by
  default it is the `appData` directory appended with your app's name.
...
`userData` path or the 'appCache' path before the `ready` event of the `app` module is emitted.
