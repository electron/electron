...
This method sets the current executable as the default handler for a protocol (aka URI scheme). It allows your app to be opened through links like `myapp://`. On Windows, you can provide optional parameters to specify the path to the Electron executable and arguments.

:::note Platform-Specific Behavior

- **Windows & Linux**: When your app is launched via a protocol link and is not running (cold start), the URL will be passed as a command line argument. You should check `process.argv` in the `ready` event to handle it.
- **macOS**: When your app is launched via a protocol link, the URL will be passed through the `open-url` event, regardless of whether the app is already running or not. You should listen for this event as early as possible, ideally in or before the `will-finish-launching` event.
  :::

For development scenarios where `process.defaultApp` is `true`, you need to pass both the path to your app and any relevant arguments:
...
See the [Deep Linking guide](../tutorial/deep-linking.md) for a complete implementation example and best practices.
...
dialog
  .showOpenDialog(null, { securityScopedBookmarks: true })
...
app.setClientCertRequestPasswordHandler(
  async ({ hostname, tokenName, isRetry }) => {
