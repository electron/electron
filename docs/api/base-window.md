...
* `phase` string - Can be `began`, `changed`, `ended`, `cancelled`, or `settling`.
...
After `ended` or `cancelled`, `settling` events report progress until it reaches
`1` or returns to `0`, respectively.
