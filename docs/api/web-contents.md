...
Returns `string` - The preferred color scheme override. Can be `inherit`, `light`
or `dark`. Defaults to `inherit` meaning that this `WebContents` will follow the
...
* `colorScheme` string (optional) - Can be `inherit`, `light` or `dark`. Defaults
  to `inherit` meaning that this `WebContents` will follow the value set by
...
without `colorScheme`, or setting it to `inherit`, removes the override.
