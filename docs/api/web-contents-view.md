...
* `options` Object (optional)
  ...
  * `paintWhenInitiallyHidden` Boolean (optional) - Whether the renderer should be active and painting even when the view is not yet visible. Setting this to `true` ensures that the `ready-to-show` event fires and `document.visibilityState` is correctly initialized during the initial load, even if the view has not been shown. Defaults to false.
