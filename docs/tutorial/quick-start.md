...
Electron is a framework that enables you to create desktop applications with JavaScript, HTML, and CSS. These applications can then be packaged to run on MacOS, Windows, or Linux or distributed via Mac and Windows application stores.
...
> Please install Node.js using pre-built installers for your platform otherwise
> you may encounter incompatibility issues with different development tools.
...
1. Line 1: First, you import `app` and `BrowserWindow` modules of the `electron` package to be able to manage your application's lifecycle events as well as create and control browser windows.
1. Line 3: After that, you define a function that creates a [new browser window](https://www.electronjs.org/docs/api/browser-window#new-browserwindowoptions) with node integration enabled, loads `index.html` file into this window (line 12, we will discuss the file later) and opens Developer Tools (line 13).
1. Line 16: You create a new browser window by invoking the `createWindow` function once the Electron application [is initialized](https://www.electronjs.org/docs/api/app#appwhenready).
1. Line 18: You add a new listener that tries to quit the application after the last window is closed. This listener ignores MacOS as it has a dedicated keyboard shortcut to quit the application.
1. Line 24: You add a new listener that creates a new browser window only if when the application has no visible windows after being activated. For example, after launching the application for the first time, or re-launching the already running application.

#### Create a web page

This is the web page you want to display once the application is initialized. This web page represents the Renderer process. There can be multiple browser windows where each window uses its independent process. Each window can optionally be granted with full access to Node.js API.
...
> NOTE: If the `main` field is omitted, Electron will attempt to load an `index.js` file.

By default, the `npm start` command runs the main script with Node.js. You need to change it to Electron as follows:
...
* **Custom APIs** for working with often-needed OS's native functions.
...
Electron APIs are assigned based on the process type. It means that some of the methods can be used from the Main or Renderer processes respectively or both. API documentation indicates the process a particular API method can be used from.
...
> NOTE: Because the Renderer processes may run untrusted code (especially from third parties), it is important to carefully validate the requests that come to the Main process.
