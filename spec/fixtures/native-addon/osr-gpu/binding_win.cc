...
napi_value ExtractPixels(napi_env env, napi_callback_info info) {
  ...
  auto sharedTextureHandle =
      NAPI_GET_PROPERTY_VALUE_STRING(textureInfo, "sharedTextureHandle");

  auto handle = reinterpret_cast<HANDLE>(std::stoull(sharedTextureHandle));
  std::cout << "ExtractPixels widgetType=" << widgetType
            << " pixelFormat=" << pixelFormat
            << " sharedTextureHandle=" << sharedTextureHandle << std::endl;
