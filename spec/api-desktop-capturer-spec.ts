...
describe('desktopCapturer', () => {
  ...
  ifit(process.platform === 'darwin')('showInactive with bringToFront should control z-order', async function () {
    ...
    try {
      ...
      expect(backW2Index).to.be.lessThan(
        backW1Index,
        'w1 should be behind w2 after showInactive({ bringToFront: false })'
      );
      ...
      expect(frontW1Index).to.be.lessThan(
        frontW2Index,
        'w1 should be in front of w2 after showInactive() with default bringToFront'
      );
    } finally {
      destroyWindows();
    }
