...
async function makeRelease (releaseToValidate) {
  if (releaseToValidate) {
    ...
    // index.json goes live once uploaded so do it as late as
    // possible to reduce the chances it contains a release
    // which fails to publish. It has to be done before the
    // final publish to ensure there aren't published releases
    // not contained in index.json, which causes other problems
    uploadIndexJson();
